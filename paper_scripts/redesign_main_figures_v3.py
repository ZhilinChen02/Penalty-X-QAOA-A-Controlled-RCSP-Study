...
Writes only to a designated review directory. No optimization, resampling,
...
def main():
    ...
    if out==ROOT or out in ROOT.parents or any((ROOT/d)==out or (ROOT/d) in out.parents for d in ['src','data','results','configs','overleaf','reproduction','tests']):
        parser.error('output must be a review directory, never a scientific-source directory')
    out.mkdir(parents=True,exist_ok=True)
    before={p:sha(ROOT/p) for p in INPUTS}
    style();ledger={}
    figure1(out,ledger);figure2(out,ledger);figure3(out,ledger);figure4(out,ledger);supplement(out,ledger)
    assert before=={p:sha(ROOT/p) for p in INPUTS}
    audit={'status':'PASS','source_data_changed':0,'scientific_values_changed':0,'optimizer_invoked':False,
        'statistical_refit':False,'width_inches':WIDTH,'minimum_nominal_font_pt':8,
        'palette':{'O0':O0,'O2':O2,'O3':O3,'reference':GRAY,'failure':FAILURE},
        'source_hashes':before,'plotted_data':ledger}
    (out/'FIGURE_V3_DATA_AUDIT.json').write_text(json.dumps(audit,indent=2)+'\n')
    print(json.dumps({'status':'PASS','output':str(out),'main_figures':4,'supplementary_figures':1}))
